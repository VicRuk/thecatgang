...
<div class="main container-fluid p-5">
    <div class="d-flex justify-content-between">
        <h1>Voluntários Adminstrativos</h1>
    </div>
    ...
    <div class="table-responsive" style="height: 70vh; overflow-y: auto;">
        ...
        if (isset($_POST["buscar"])) {
            ...
            $query = mysqli_query($conexao, "SELECT * FROM user WHERE nome LIKE '%$varBuscar%' AND tipo = 'Administrativo' AND user_status='Aprovado' ORDER BY id DESC");
        } else {
            $query = mysqli_query($conexao, "SELECT * FROM user WHERE tipo = 'Administrativo' AND user_status='Aprovado' ORDER BY id DESC");
        }
    ...
    <div class="table-responsive" style="height: 90vh; overflow-y: auto;">
        ...
        if (isset($_POST["buscar"])) {
            $varBuscar = $_POST["buscar"];
            $query = mysqli_query($conexao, "SELECT * FROM user WHERE nome LIKE '%$varBuscar%' AND user_status = 'Em análise' AND tipo = 'Administrativo' ORDER BY id DESC");
        } else {
            $query = mysqli_query($conexao, "SELECT * FROM user WHERE user_status = 'Em análise' AND tipo = 'Administrativo' ORDER BY id DESC");
        }
